...
import { hasEngineFlag, resolveResourceKey, LINES, isUndoable, findEntity, canAffordCost, getAttackCost, isEntityOnBoard, isAutoCast, findEntityLocation, getAvatar, getOwnerId, log, moveEntity, payCost, warn } from './utils.js';
import { ACTION_MANIFEST, EVENT_MANIFEST, ACTION_REGISTRY } from './actions/core.js';
import { ATTRIBUTE_MANIFEST } from './attributes.js';
import { shuffleArray as prandomShuffle } from './prandom.js';
...
export class GameEngine {
    ...
    executeAbility(ability, source, eventPayload, ownerId, originatingEvent = null) {
        ...
        try {
            ...
            // 1. Lock targets FIRST
            const lockedTargets = this._acquireTargets(ability, source, eventPayload, ownerId);

            // 2. Pre-flight Check: Ensure at least one effect group has valid targets
            const hasValidTargets = ability.effects.some((group, index) => {
                if (!group) return false;
                if (!group.targetMethod || group.targetMethod === 'NONE') return true;
                return lockedTargets[index] && lockedTargets[index].length > 0;
            });

            if (!hasValidTargets && ability.effects.length > 0) {
                log(this.state, `[ABILITY DEBUG] result=ABORT reason=no-targets ability='${ability.name || ability.abilityId}' source=${sId}`);
                return; // Abort silently without consuming limits or resources
            }

            // 3. Now check and pay the cost / limit
    ...
    _checkAndPayCost(ability, source, ownerId, eventPayload) {
        // Track limits across the entity rather than the transient ability instance
    ...
    _acquireTargets(ability, source, eventPayload, ownerId) {
        return ability.effects.map((group, index) => {
            ...
            if (group.targetMethod === 'SELF') targets = [source];
                ...
                const loc = findEntityLocation(this, source);
                if (loc && loc.zone === 'attachment' && loc.host) targets = [loc.host];
            }
                ...
                if (group.targetMethod === 'AUTO_ALL') targets = pool;
                    ...
                    targets = this.utils.shuffleArray(this.state, [...pool]).slice(0, group.targetCount || 1);
                }
    ...
    evaluateLogicTree(node, entity, source, eventPayload, lkiAbilities = null) {
        ...
        if (node.type === 'group') {
            ...
            if (checkAttr === 'entity') {
                ...
                const hasEffect = targetEnt.activeEffects?.some(e => e.type === 'GRANT_ABILITY' &&
