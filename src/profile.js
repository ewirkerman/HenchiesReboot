 ...
 */
export function resolveNotificationTargetUrl(rawUrl, baseUrl = import.meta.url) {
    ...
    if (/^https?:\/\//i.test(baseUrl)) return new URL(cleanedUrl, baseUrl).href;

    return new URL('../' + cleanedUrl, baseUrl).href;
}

// REMOVED `messaging` requirement here. `firebase.js` handles it.
export function listenForForegroundNotifications(onNotificationReceived, legacyCallback) {
    if (typeof onNotificationReceived !== 'function') onNotificationReceived = legacyCallback;
