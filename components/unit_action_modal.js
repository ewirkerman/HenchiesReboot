    ...
    connectedCallback() {
        ...
        <div id="unit-action-modal" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm hidden">
            <div class="glass-panel rounded-2xl p-5 shadow-2xl border border-slate-700 max-w-sm w-full mx-4 flex flex-col gap-4 relative">
                <button onclick="document.querySelector('unit-action-modal').close()" class="absolute top-3 right-3 text-slate-400 hover:text-white font-bold text-lg leading-none">&times;</button>
                ...
                <button onclick="document.querySelector('unit-action-modal').close()" class="bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-bold p-3 rounded-xl border border-slate-700 mt-2 transition">Cancel [ESC]</button>
            </div>
        </div>
        `;
    }
